...
-- 1. PRE-AUTHORIZATION KANBAN BOARD QUERY
-- Efficiently group and load treatments pending insurance approval by status
-- Uses the 'idx_pre_auths_kanban' composite index for optimal sorting and filtering.
EXPLAIN ANALYZE
SELECT 
    pa.id,
    pa.status,
...
ORDER BY pa.status, pa.updated_at DESC;


-- 2. PREVENTING N+1: RETRIEVING PATIENTS WITH UPCOMING APPOINTMENTS & SMS REMINDERS
...
-- 3. THE RECALL SELECTOR (Finding candidates for automated text messages)
...
-- 4. VACANT SELF-BOOKING SLOTS FOR WIDGET
